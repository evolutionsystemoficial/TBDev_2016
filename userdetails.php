...
function maketable($res){
    ...
    $HTMLOUT .= "
                 <div class='cblock'>
                     <div class='cblock-header'>" . format_username($user, true) . "$country</div>";
...
//}
    ...
    $HTMLOUT .= "<tr><td class='rowhead'>{$lang['userdetails_class']}</td><td align='left'><img src=" . get_user_class_image($user["class"]) . " " . get_user_class_name($user["class"]) . "</td></tr>\n";
    ...
    if ($CURUSER['class'] >= UC_MODERATOR && $user["class"] < $CURUSER['class']){
      ...
      $HTMLOUT .= "<tr><td class='rowhead'" . (!$warned ? " rowspan='2'": "") . ">{$lang['userdetails_warned']}</td><td align='left' width='20%'>" .( $warned ? "<input name=warned value='yes' type='radio' checked='checked' />{$lang['userdetails_yes']}<input name='warned' value='no' type='radio' />{$lang['userdetails_no']}" : "{$lang['userdetails_no']}" ) ."</td>";
      if ($warned){
