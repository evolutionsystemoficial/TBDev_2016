...
if ( mysql_num_rows( $res ) == 0 )
   ...
   elseif($arr['to_user'] == $CURUSER['id'] || ($arr['userid'] == $CURUSER['id'] && $arr['to_user'] !=0) )
      $private = "<a href=\"javascript:private_reply('".$arr['username']."')\"><img src=\"{$TBDEV['pic_base_url']}Reply-icon.png\" alt=\"Private shout\" title=\"Private shout! click to reply to ".$arr['username']."\" width=\"16\" style=\"padding-left:2px;padding-right:2px;\" border=\"0\" /></a>";
   else
      $private = '';
        $edit = ($CURUSER['class'] >= UC_MODERATOR || ($arr['userid'] == $CURUSER['id']) && ($CURUSER['class'] >= UC_POWER_USER && $CURUSER['class'] <= UC_STAFF) ? "<a href='{$TBDEV['baseurl']}/shoutbox.php?edit=" . $arr['id'] . "&amp;user=".$arr['userid']."'><img src='{$TBDEV['pic_base_url']}Edit-Text.png' border='0' alt=\"Edit Shout\"  title=\"Edit Shout\" /></a> " : "" );
        $del = ( $CURUSER['class'] >= UC_MODERATOR ? "<a href='./shoutbox.php?del=" . $arr['id'] . "'><img src='{$TBDEV['pic_base_url']}delsingle.png' border='0' alt=\"Delete Single Shout\" title=\"Delete Single Shout\" /></a> " : "" );
        $delall = ( $CURUSER['class'] >= UC_SYSOP ? "<a href='./shoutbox.php?delall' onclick=\"confirm_delete(); return false;\"><img src='{$TBDEV['pic_base_url']}delete-icon.png' border='0' alt=\"Empty Shout\" title=\"Empty Shout\" /></a> " : "" );      
        $pm = "<span class='date' style=\"color:$dtcolor\"><a target='_blank' href='./sendmessage.php?receiver=$arr[userid]'><img src='{$TBDEV['pic_base_url']}Mail-Reply.png' border='0' alt=\"Pm User\" title=\"Pm User\" /></a></span>\n";
